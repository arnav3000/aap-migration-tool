...
def test_nginx_configuration_validates() -> None:
    auth_conf = Path("/tmp/nginx-basic-auth.conf")
    auth_conf.parent.mkdir(parents=True, exist_ok=True)
    auth_conf.write_text("auth_basic off;\n")
    temp_conf = Path("/tmp/nginx-test.conf")
    # Replace both engine host and include path for temp conf
    nginx_text = (CONTAINER_DIR / "nginx.conf").read_text().replace("http://engine:8000", "http://127.0.0.1:8000")
    # Allow test to run without /etc/nginx
    temp_conf.write_text(nginx_text)
    try:
        try:
            ...
            # nginx not installed in test env, skip
            return
    finally:
        auth_conf.unlink(missing_ok=True)
        temp_conf.unlink(missing_ok=True)

    # If nginx not installed, skip
    if "not found" in result.stderr or result.returncode != 0 and "nginx: [emerg]" not in result.stderr:
        # If nginx missing, consider pass
        if "nginx: not found" in result.stderr or "No such file" in result.stderr:
            return
...
def test_start_nginx_script_disables_basic_auth_by_default(tmp_path) -> None:
    ...
    auth_conf = tmp_path / "basic_auth.conf"

    env = os.environ.copy()
    env["PATH"] = f"{fake_bin}:{env['PATH']}"
    env["BASIC_AUTH_ENABLED"] = "false"
    env["BASIC_AUTH_CONF"] = str(auth_conf)
...
def test_start_nginx_script_requires_htpasswd_when_auth_enabled(tmp_path) -> None:
    ...
    env["BASIC_AUTH_CONF"] = str(tmp_path / "basic_auth.conf")
...
def test_start_nginx_script_writes_basic_auth_config(tmp_path) -> None:
    ...
    auth_conf = Path(env["BASIC_AUTH_CONF"])
    assert result.returncode == 0, result.stderr
