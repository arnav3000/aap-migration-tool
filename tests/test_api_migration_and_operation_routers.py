...
async def test_migration_router_preview_run_and_state(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # For run, limit to same types so test's fake clients don't need to handle all types
    monkeypatch.setattr("aap_migration.resources.get_fully_supported_types", lambda: ["organizations", "users"])
    monkeypatch.setattr("aap_migration.resources.get_exportable_types", lambda: ["organizations", "users"])
    ...
    # Add get to source as well for bootstrap/export
    async def _fake_get(self, endpoint, **kwargs):
        return {"results": [], "count": 0}
    FakeSourceClient.get = _fake_get
    ...
    assert (
        "users" not in preview_result["resources"] or len(preview_result["resources"]["users"]) in (1, 2)
    )
    ...
    # For run, limit to organizations only so users are excluded from events (matches test expectation)
    monkeypatch.setattr("aap_migration.resources.get_fully_supported_types", lambda: ["organizations"])

    # Mock heavy planner helpers for run so fake clients suffice
    import aap_migration.api.routers.planner as planner_module

    def _fake_build_source_contexts(source_configs, dest_cfg, dest_auth, db_url):
        class _DummyClient:
            async def __aenter__(self):
                return self
        ...
        # Minimal sources structure expected by _migrate_resource_type
        fake_sources = [
            {
                "excluded_ids": {"users": [2]},
                "preview_resources": {},
                "src_client": _DummyClient(),
                "state": type("S", (), {"get_mapped_id": lambda *a, **k: None})(),
            }
        ]
        return dest_cfg, _DummyClient(), fake_sources

    async def _fake_migrate_resource_type(rtype, sources, target_client, phase_num, emit, log, created_creds):
        # Simulate run: organizations creates 1, users are excluded
        if rtype == "users":
            return (0, 0, 0, 0)
        if rtype == "organizations":
            emit({"_event": "phase_start", "phase_num": phase_num, "description": "Organizations", "resource_type": rtype})
            emit({"_event": "phase_complete", "phase_num": phase_num, "description": "Organizations", "created": 1, "updated": 0, "skipped": 0, "failed": 0, "exported": 1, "duration": "0.0s", "warnings": {}})
            return (1, 0, 0, 1)
        # other types do nothing
        return (0, 0, 0, 0)

    async def _fake_run_cac(*a, **k):
        return 0

    async def _fake_handle_pause(*a, **k):
        return None

    monkeypatch.setattr(planner_module, "_build_source_contexts", _fake_build_source_contexts)
    monkeypatch.setattr(planner_module, "_migrate_resource_type", _fake_migrate_resource_type)
    monkeypatch.setattr(planner_module, "_run_cac_org_update", _fake_run_cac)
    monkeypatch.setattr(planner_module, "_handle_credential_pause", _fake_handle_pause)
    # also patch migration's view of these (it imports inside function, so patching planner is enough)
    ...
    assert run_result["total_created"] == 1
    assert run_result["total_skipped"] == 0
    assert run_result["total_failed"] == 0
