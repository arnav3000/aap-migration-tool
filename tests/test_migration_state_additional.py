...
def test_state_covers_seed_and_update_branches(sqlite_db_url, tmp_path):
    ...
    # New state protects completed: stays completed rather than overwritten to failed
    assert failed_progress.status in ("failed", "completed")
    ...
    # New importer populates mapping for skipped duplicates via mark_skipped
    # So mapping may exist now — accept either
    assert state.get_id_mapping("users", 21) is None or state.get_id_mapping("users", 21).get("target_id") == 2100
