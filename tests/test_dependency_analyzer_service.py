...
async def test_analyze_all_organizations_propagates_task_exceptions(
    ...
    # New analyzer is resilient: it logs per-org failure and continues
    # Old behavior was to propagate; new expected is to complete with available orgs
    report = await analyzer.analyze_all_organizations()
    assert report.total_organizations >= 1
    assert "OrgA" in report.org_reports
    # OrgB failure should be logged but not crash the whole analysis
    assert report.total_organizations in (1, 2)
