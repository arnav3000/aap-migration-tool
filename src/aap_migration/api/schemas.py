...
class ValidateRunRequest(BaseModel):
    ...
    Mirrors CLI flags in ``cli/commands/validate.py`` — ``live``,
    ``resource_type``, ``skip_hosts``, ``orgs`` etc.  Both CLI and API
    call the same engine (``validate/runner.py:run_validation``) so
    behaviour is identical.  ``source_id`` / ``destination_id`` are
    optional connection IDs; when provided they supply URLs/tokens for
    the report header and live target fetch.  When omitted the server
    falls back to ``MIGRATION_STATE_DB_PATH`` / ``config/*.yaml`` or
    placeholder URLs.
    """
...
class IAMBenchmarkRequest(BaseModel):
    """IAM benchmark — reuses ``iam/benchmark.py:run_benchmark``."""
...
class IAMReportRequest(BaseModel):
    """Regenerate HTML from a previous JSON export."""
