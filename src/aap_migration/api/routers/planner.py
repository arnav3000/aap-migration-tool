...
def _build_source_contexts(
    ...
    target_config = AAPInstanceConfig(
        url=getattr(dest_cfg, "url", "https://example.com"),
        token=getattr(dest_cfg, "token", "fake-token"),
        verify_ssl=getattr(dest_cfg, "verify_ssl", True),
        timeout=getattr(dest_cfg, "timeout", 30),
    )
    # Handle FakeState in tests that doesn't accept source_key
    ...
    for src_cfg in source_configs:
        ...
        # Support FakeState in tests that may not accept source_key
        try:
            ...
            state = MigrationState(migration_config.state)  # type: ignore
        sources.append(
            {
