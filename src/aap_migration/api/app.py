...
def create_app(db_url: str | None = None) -> FastAPI:
    ...
    if not effective_url.startswith(("sqlite://", "postgresql://", "mysql://")):
        # Tests and simple CLI invocations may pass a bare file path like "relative.db"
        # Normalize to a SQLite URL for backward compatibility.
        # Production should still use a full DSN.
...
def _seed_connections_from_env(session_factory: sessionmaker) -> None:
    ...
    try:
        ...
        roles_to_seed: list[tuple[str, str, str, bool]] = [
            ...
            ("target", "TARGET__", "aap", has_destination),
        ]
